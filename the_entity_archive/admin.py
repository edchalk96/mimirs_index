...
class EntityAdmin(SummernoteModelAdmin):
    ...
    list_display = ('name', 'status', 'author__username', 'lore_count', 'is_deletion_pending')
    ...
    def approve_deletion (self, request, queryset):
        ...
        to_delete.delete()
        self.message_user(request, f"Successfully removed {count} entities from the archive")
    ...
    def publish_entity (self, request, queryset):
        ...
        self.message_user(request, f"Successfully published {count} entities to the archive")
